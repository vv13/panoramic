 ...
 **/
/* eslint-disable */
import PSVNavBarButton from './PSVNavBarButton'
import Sphoords from './sphoords'

export default function PSVNavBar(psv) {
	...
	var checkValue = function(property, value) {
		return (
				// Color
				(
					colors.includes(property) && (typeof value == 'string') &&
					(
						value == 'transparent' ||
				...
				// Number
				(numbers.includes(property) && !isNaN(parseFloat(value)) && isFinite(value) && value >= 0)
			);
	};
